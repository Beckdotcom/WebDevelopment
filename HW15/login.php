...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if (was_taken($username)){
            echo "<p>You've already taken the test!</p>";
        }
        else if(search($username,$password)){
            //save as cookie,
            //$_SESSION["username"] = $username;
            //redirects. header for http response
            setcookie("username",$username,time()+960,"/");

...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(search($username,$password)){
            ...
            echo "<script> alert( \"<p>Login Failed</p>\")</script>";
            //echo "pass".$password2;
            //echo "line".$line;
            //echo "user".$username2;
            //echo "pass".$password;
    ...
    function search($username,$password){
        $myfile = fopen("../dbase/passwd.txt", "r") or die("Unable to open file!");
        $line = fgets($myfile);
        while($line != ""){
    ...
    <form method="post" action="login.php">
        <span id="alerts">  </span>
        <div> Username: <input type="text" name="username" required autofocus> </div>
        <div> Password: <input type="password" name="password" required autofocus> </div>
